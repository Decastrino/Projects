...
#perform file system operations such as permissions
#get owner, size calculations
...
class commandsClass(object):
    ...
    def grep(command_passed, command_length):
	    ...
	    #get second and third arguments
            second_argument = command_passed[1]
            third_argument = command_passed[2]
			
	    #open the file for reading
            with open(third_argument, 'r') as searchfile:
                for line in searchfile:
                    if second_argument in line:
                        print (line)
                    else:
                        pass
    ...
    # Code not working correctly as expected. Works better on a Unix.
